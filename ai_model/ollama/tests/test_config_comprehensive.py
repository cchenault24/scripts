...
class TestNormalizeModel:
    ...
    def test_normalize_recommended_model(self):
        ...
        assert result["ollama_name"] == "test:latest"
...
class TestIsOurFile:
    ...
    def test_file_with_fingerprint(self, tmp_path):
        """Test detecting our file by fingerprint."""
        test_file = tmp_path / "config.yaml"
        test_file.write_text("# Generated by ollama-llm-setup.py v2.0.0\nkey: value")
...
class TestGenerateContinueConfig:
    ...
    def test_generates_valid_config(self, mock_success, mock_header, tmp_path):
        ...
        models = [
            RecommendedModel("Chat", "chat:v", 5.0, ModelRole.CHAT, ["chat", "edit"]),
            RecommendedModel("Embed", "embed:v", 0.3, ModelRole.EMBED, ["embed"]),
        ]
...
class TestGenerateGlobalRule:
    ...
    def test_generates_rule_file(self, mock_info, mock_success, mock_header, tmp_path):
        ...
        assert result.exists()
        content = result.read_text()
        # Should be markdown with instructions
