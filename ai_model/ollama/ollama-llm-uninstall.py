#!/usr/bin/env python3
"""
Ollama + Continue.dev Uninstaller
...
# Ensure the ollama_dir is in sys.path
...
def check_ollama() -> Tuple[bool, str]:
    """Check if Ollama is installed and running. Returns (success, message)."""
    if not shutil.which("ollama"):
        return False, "Ollama command not found"
    
    # Use shorter timeout for quick check
    code, stdout, stderr = run_command(["ollama", "--version"], timeout=5, show_progress=False)
    if code == 0:
        return True, "Ollama is installed"
    elif code == -1:
        return False, "Ollama check timed out"
    else:
        return False, f"Ollama check failed: {stderr[:100] if stderr else 'Unknown error'}"


def check_ollama_api() -> Tuple[bool, str]:
    """Check if Ollama API is available. Returns (success, message)."""
    # Use shorter timeout for quick check
    try:
    ...
    code, _, stderr = run_command(["ollama", "list"], timeout=5, show_progress=False)
    if code == 0:
        return True, "Ollama is available"
    elif code == -1:
        return False, "Ollama API check timed out"
    else:
        return False, f"Ollama API check failed: {stderr[:100] if stderr else 'Unknown error'}"


def list_ollama_models() -> List[str]:
    """List all installed Ollama models."""
    code, stdout, _ = run_command(["ollama", "list"], timeout=10, show_progress=False)
    if code != 0:
        return []
...
def remove_ollama_model(model_name: str) -> bool:
    ...
    code, stdout, stderr = run_command(["ollama", "rm", model_name], timeout=600)
    if code == 0:
        return True
    else:
...
def is_generated_config(config_path: Path) -> bool:
    """Check if a config file was generated by ollama-llm-setup.py."""
    ...
    try:
        with open(config_path, "r") as f:
            ...
            if config_path.suffix in (".yaml", ".yml"):
                return "Generated by ollama-llm-setup.py" in content or "Ollama" in content
            
            # For JSON files, check for Ollama API endpoint pattern
            elif config_path.suffix == ".json":
                # Check if it contains Ollama API endpoint
                # Ollama typically uses localhost:11434
                return ("localhost:11434" in content or 
                        ":11434" in content)
...
def uninstall_models(models: List[str]) -> int:
    ...
    print_subheader("Removing Ollama Models")
...
def uninstall_config_files(restore_backup: bool = False, warn_shared: bool = True) -> bool:
    ...
    if config_yaml and config_yaml.exists():
        if is_generated_config(config_yaml):
            ...
            print_warning(f"{config_yaml} exists but doesn't appear to be generated by ollama-llm-setup.py")
    ...
    if config_json and config_json.exists():
        if is_generated_config(config_json):
            ...
            print_warning(f"{config_json} exists but doesn't appear to be generated by ollama-llm-setup.py")
            if prompt_yes_no("Remove it anyway?", default=False):
                try:
                    config_json.unlink()
                    print_success(f"Removed {config_json}")
...
def show_summary(
    ...
    print("  • Ollama Desktop remains installed")
...
def main() -> int:
    ...
    print_header("🗑️  Ollama + Continue.dev Uninstaller")
    print_info("This script will help you remove components installed by ollama-llm-setup.py")
    print()
    ...
    # Check Ollama (with option to skip if hanging)
    if args.skip_ollama_checks:
        print()
        print_subheader("Checking Ollama")
        print_warning("Skipping Ollama checks (--skip-ollama-checks flag used)")
        ollama_ok = False
        ollama_available = False
    else:
        print()
        print_subheader("Checking Ollama")
        print_info("Checking Ollama status (this may take a few seconds)...")
        try:
            ollama_ok, ollama_msg = check_ollama()
            if ollama_ok:
                print_success(ollama_msg)
            else:
                print_warning(ollama_msg)
                print_info("Some operations may be skipped")
        except KeyboardInterrupt:
            print()
            print_warning("Ollama check interrupted")
            if prompt_yes_no("Skip Ollama checks and continue?", default=True):
                ollama_ok = False
                ollama_msg = "Skipped"
            else:
                print_info("Uninstallation cancelled.")
        ...
        # Check Ollama API (with option to skip if hanging)
        if ollama_ok:
            print_info("Checking Ollama API...")
            try:
                ollama_available, ollama_api_msg = check_ollama_api()
                if ollama_available:
                    print_success(ollama_api_msg)
                else:
                    print_warning(ollama_api_msg)
                    print_info("Model removal will be skipped")
            except KeyboardInterrupt:
                print()
                print_warning("Ollama API check interrupted")
                if prompt_yes_no("Skip Ollama API checks and continue?", default=True):
                    ollama_available = False
                    ollama_api_msg = "Skipped"
                else:
                    print_info("Uninstallation cancelled.")
                    return 0
        else:
            ollama_available = False
            ollama_api_msg = "Ollama not available"
    
    # Step 1: Remove Ollama models
    models_removed = 0
    if args.skip_models:
        print()
        print_subheader("Ollama Models")
        print_warning("Skipping Ollama model removal (--skip-models flag used)")
        ...
        if models:
            ...
            if choice == 0:  # Remove all
                if prompt_yes_no("Remove all Ollama models?", default=False):
                ...
                print_info("Keeping all Ollama models")
        else:
            print_info("No Ollama models found")
    else:
        print()
        print_subheader("Ollama Models")
        print_info("Ollama API not available - skipping model removal")
