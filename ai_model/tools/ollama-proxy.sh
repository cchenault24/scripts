#!/bin/bash
#
# ollama-proxy.sh - Optimization Proxy for Continue.dev/Ollama
#
# This proxy sits between Continue.dev and Ollama, intercepting requests
# and applying optimizations (routing, queuing, performance tracking)
#
# Usage:
#   ./tools/ollama-proxy.sh [port] [ollama_port]
#   Default: port=11435, ollama_port=11434
#
# Then update Continue.dev config to use: apiBase: http://localhost:11435
...
# Detect hardware tier if not set
if [[ -z "${HARDWARE_TIER:-}" ]]; then
  detect_hardware >/dev/null 2>&1 || true
fi

print_header "🚀 Ollama Optimization Proxy"
print_info "Proxy listening on port $PROXY_PORT"
print_info "Forwarding to Ollama on port $OLLAMA_PORT"
print_info "Optimizations: ENABLED"
echo ""

# Simple HTTP server using netcat (macOS compatible)
# This is a basic implementation - for production, consider using a proper HTTP server
listen_and_process() {
  while true; do
    # Wait for connection (basic implementation)
    # Note: This is simplified - a production proxy would use a proper HTTP server
    print_info "Proxy ready. Connect Continue.dev to: http://localhost:$PROXY_PORT"
...
# Alternative: Use Python/socat for proper HTTP proxy
# Check if Python is available for a better proxy implementation
if command -v python3 &>/dev/null; then
  print_info "Python detected - using enhanced proxy"
  
  # Create Python proxy script with proper variable expansion
  cat > "$SCRIPT_DIR/ollama_proxy_server.py" <<EOF
#!/usr/bin/env python3
"""
...
EOF
  ...
  chmod +x "$SCRIPT_DIR/ollama_proxy_server.py"
  
  # Set environment and run
  export OLLAMA_PORT="$OLLAMA_PORT"
  export PROXY_PORT="$PROXY_PORT"
  export PROJECT_DIR="$PROJECT_DIR"
  export HARDWARE_TIER="${HARDWARE_TIER:-B}"
  
  print_success "Starting Python proxy server..."
  python3 "$SCRIPT_DIR/ollama_proxy_server.py"
  
else
  print_warn "Python 3 not found. Using basic proxy (limited functionality)"
  print_info "Install Python 3 for full optimization support:"
