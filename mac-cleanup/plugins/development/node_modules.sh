...
clean_node_modules() {
  ...
  if [[ "$MC_DRY_RUN" != "true" ]] && gum confirm "Do you want to clean node_modules directories? (WARNING: Requires re-install)"; then
  ...
  if [[ $total_space_freed -eq 0 ]]; then
    ...
    track_space_saved "Node.js Modules" $total_space_freed
