...
**Single Backend Design** (v4.1+): The project uses Ollama exclusively for local LLM hosting. Previously supported dual backends (Docker + Ollama) but simplified to Ollama-only in v4.1 for better maintainability and reduced code duplication.

**Module Structure**:
- `lib/`: Core library modules (11 Python files)
  - `config.py`, `hardware.py`, `ide.py`, `model_selector.py`, `models.py`
  - `ollama.py`, `ui.py`, `uninstaller.py`, `utils.py`, `validator.py`
- `tests/`: Unified test suite (21 test files)
- `setup.py`: Main entry point (was `ollama/ollama-llm-setup.py`)
- `uninstall.py`: Uninstaller (was `ollama/ollama-llm-uninstall.py`)

**Key Design Principles**:
- **Fixed Model Selection**: GPT-OSS 20B (16GB) + nomic-embed-text (0.3GB) for all 16GB+ RAM systems
- **Apple Silicon Only**: Code requires Apple Silicon detection (M1/M2/M3/M4)
- **No Abstraction Overhead**: Single backend eliminates architectural complexity
...
- Unified test suite in `tests/` directory (21 test files)
- TDD approach with `SPECIFICATIONS.md` defining behavioral specs
...
**Model Selection**: The `model_selector.py` and `validator.py` modules implement **fixed model selection**:
- All users get GPT-OSS 20B (16GB) + nomic-embed-text (0.3GB)
- No hardware-based tiering or user choice
- Simplified setup flow
- Requires Apple Silicon (M1/M2/M3/M4) - exits on non-Apple hardware

**VPN/SSH Handling**: Ollama has special SSH_AUTH_SOCK handling due to a Go HTTP library bug. When calling Ollama commands, the environment is cleaned to remove SSH_AUTH_SOCK for VPN resilience.

**Progress Bars**: Supports rich terminal progress bars with automatic fallback to basic output if `rich` library is unavailable.

**Architecture History**: Previously supported dual backends (Docker + Ollama) with 60-70% code duplication. Simplified to Ollama-only in v4.1 (April 2026) to reduce complexity and maintenance burden. See `REFACTORING_NOTES.md` for migration details.
